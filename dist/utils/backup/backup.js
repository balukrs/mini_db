...
async function backupFilesFolders(source, destination) {
    const timestamp = getTimestamp();
    const fileHandle = await fs.open(`${destination}/backup_${timestamp}.zip`, 'w');
    ...
    archive.on('data', (chunk) => {
        ...
        let progress = Math.min(Math.round((totalProcessed / totalSize) * 100), 100);
