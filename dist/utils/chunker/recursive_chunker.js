...
function recursiveChunker(content, chunk_size, overlap, seperators) {
    const chunks = _recursiveChunker(content, chunk_size, seperators);
    if (overlap > 0) {
        for (let i = 1; i < chunks.length; i++) {
            const prevData = chunks[i - 1]?.slice(-overlap) || '';
...
//# sourceMappingURL=recursive_chunker.js.map